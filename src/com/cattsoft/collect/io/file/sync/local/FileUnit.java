...
class FileUnit {
	...
	/**
	 * file/directory name (ends with a trailing separator only if this is a
	 * directory)
	 */
	final String name;

	/**
	 * size of file/directory in bytes (size of directory is fixed at zero
	 * bytes)
	 */
	final long size;

	/**
	 * last-modified time in milliseconds since the epoch (00:00:00 GMT, January
	 * 1, 1970)
	 */
	final long time;
	...
	/**
	 * CRC-32 checksum of file/directory (CRC-32 checksum of directory is fixed
	 * at zero)
	 */
	private long crc = 0L;

	/**
	 * FileUnit object representing the corresponding matching target/source
	...
	/**
	 * true if the corresponding matching target/source file/directory has the
	 * same last-modified time; false otherwise
	 */
	boolean sameTime = false;

	/**
	 * true if the corresponding matching target/source file/directory has the
	 * same CRC-32 checksum; false otherwise
	 */
	 ...
	 * 
	 * @param f
	 *            File object representing this file/directory (full canonical
	 *            pathname)
	 */
	FileUnit(final File f) {
		...
		if (this.isDirectory) {
			this.name = SyncIO.trimTrailingSeparator(f.getName())
					+ File.separatorChar;
			this.size = 0L;
			this.crc = 0L;
	...
	public long getCrc() {
		if (!this.computedCrc) {
			final SyncIO.Crc32Checksum result = SyncIO
					.getCrc32Checksum(this.file);
	...
	public String getSizeString() {
		if (this.sizeString == null) {
			if (this.size < 1024) {
				...
				this.sizeString = String.format("%.1f Mb",
						this.size / 1048576.0);
			} else {
				this.sizeString = String.format("%.1f Gb",
						this.size / 1073741824.0);
			}
		}
	...
	public String getTimeString() {
		if (this.timeString == null)
			this.timeString = String.format(Locale.ENGLISH,
					Sync.TIME_FORMAT_STRING, new Date(this.time));
	...
	public String getCrcString() {
		if (this.crcString == null)
			this.crcString = Long.toHexString(this.getCrc()).toUpperCase(
					Locale.ENGLISH);
